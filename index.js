...
const roleId = '1192744421201035374'; // Remplacez par l'ID du rôle que vous souhaitez attribuer
const mentionnedrole = "<@&1192744421201035374>"
  ...
  if (message.content.startsWith(config.prefix + "addrole") && message.guild && !message.author.bot) {
    ...
    collector.on('collect', async (reaction) => {
      ...
      if (reaction.emoji.name === addEmoji) {
        ...
        const replyMessage = await message.reply(`Le rôle ${mentionnedrole} vous a été ajouté avec succès.`);
        setTimeout(() => {
          replyMessage.delete().catch(console.error);
        }, 5000); // Supprime le message après 5 secondes
      } else if (reaction.emoji.name === removeEmoji) {
        member.roles.remove(roleId);
        const replyMessage = await message.reply(`Le rôle ${mentionnedrole} vous a été retiré avec succès.`);
        setTimeout(() => {
          replyMessage.delete().catch(console.error);
        }, 5000); // Supprime le message après 5 secondes
      }
